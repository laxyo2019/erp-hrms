	...
	<div style=" padding: 1.5rem; border: 1px solid white;background: white">
		{{-- <div class="col-md-12 col-xl-12"> --}}
			<h1 style="font-size: 24px">Add Allowance Here
				<a href="{{ route('by-employee.index') }}" class="btn btn-sm btn-primary pull-right"  style="{background-color: #e7e7e7; color: black;}" >Go Back</a>
		</h1>
		{{-- </div> --}}
		...
		<form action="{{route('by-employee.store')}}" method="POST" >
			@csrf
			<hr>
			<div class="row">
					...
					<select name="employee" class="custom-select form-control select2">
						<option value="">Select Type</option>
							@foreach($employees as $index)
								<option value="{{$index->user_id}}">{{ucwords($index->emp_name)}}</option>
							@endforeach
					</select>
					@error('employee')
		                  <span class="text-danger" role="alert">
